...
void Generator::addExtraPassages(Maze& maze) {
    // Плотность 1.0 — идеальный лабиринт, ничего не ломаем
    if (wallDensity >= 1.0) {
        return;
    }
    ...
    // Собираем все стены, которые ещё стоят
    ...
    // Перемешиваем — порядок удаления должен быть случайным
    std::random_device rd;
    std::mt19937 rng(rd());
    std::shuffle(possibleWalls.begin(), possibleWalls.end(), rng);

    // Считаем, сколько стен убрать: при плотности 0.5 — половину, при 0 — все
    ...
    for (size_t i = 0; i < wallsToRemove && i < possibleWalls.size(); ++i) {
        ...
        // Случайно выбираем: East или South
